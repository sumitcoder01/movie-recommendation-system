...
if st.button('Recommend'):
    ...
    col1.image(load_image(recommend_movie[0][1]), width=150, caption='Image not found' if recommend_movie[0][1] is None else None)

    col2.header(recommend_movie[1][0])
    col2.image(load_image(recommend_movie[1][1]), width=150, caption='Image not found' if recommend_movie[1][1] is None else None)

    col1, col2 = st.columns(2, gap="medium")

    col1.header(recommend_movie[2][0])
    col1.image(load_image(recommend_movie[2][1]), width=150, caption='Image not found' if recommend_movie[2][1] is None else None)

    col2.header(recommend_movie[3][0])
    col2.image(load_image(recommend_movie[3][1]), width=150, caption='Image not found' if recommend_movie[3][1] is None else None)

    st.header(recommend_movie[4][0])
    st.image(load_image(recommend_movie[4][1]), width=150, caption='Image not found' if recommend_movie[4][1] is None else None)
else:
